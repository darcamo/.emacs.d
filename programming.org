...
* Senator with hide-show (define a hydra)                    :Semantic:HideShow:
  ...
  #+begin_src emacs-lisp
    ...
    (defhydra hydra-senator-hideshow (:hint nil)
              "
              Senator/hide-show:
                    _j_: (n) next tag         _u_: up tag    _q_uit
                    _k_: (p) previous tag     _r_: Copy to register

                    _w_: copy tag             _y_: yank
                    _,_: Transpose up         _._: Transpose down
    ...
    (add-hook 'semantic-mode-hook
              (lambda ()
                (local-set-key "\C-cs" 'hydra-senator-hideshow/body)
                ))
  #+end_src
