...
*** COMMENT Smex (Ido-like completion for Commands)                       :Smex:
    ...
    :TANGlE:   no
    :END:
    NOTA: Hoje em dia não uso mais o smex. Uso o helm-M-x no lugar.
...
* fm-bookmarks
  ...
  #+begin_src emacs-lisp
    (use-package fm-bookmarks
      ...
      (global-set-key (kbd "C-c b") #'fm-bookmarks)
      ;; Use ` to open FM Bookmarks in Dired-mode
      (define-key dired-mode-map (kbd "b") #'fm-bookmarks)
      ...
      (setq fm-bookmarks-enabled-file-managers '(gnome3 custom media))
...
*** Volatile Highlights
    ...
    #+end_src
* Ag searcher
  Ag (silversearcher-ag) is a VERY nice searcher (better than using grep).
  It's very fast and it will also ignore backups and directories associated
  with version control files, such as .git and .svn as well as files that
  are ignored by these version control (added to .gitignore, for example).

  The emacs Ag package defines some commands to running ag inside Emacs.
  Documentation in http://agel.readthedocs.io/en/latest/usage.html

  In the search buffer you can use 'n' and 'p' to navigate, 'enter'to visit
  the file and 'q' to close th buffer.
  #+begin_src emacs-lisp
    (use-package ag
      :ensure t
      )
  ...
  *NOTE:* You might also try installing the helm-ag package for a helm-like
   experience.
* Swiper
  #+begin_src emacs-lisp
    (use-package swiper
    ...
    (with-library 'diminish
        ...
        (with-eval-after-load "python-cell"
         ...
         (with-eval-after-load "which-key"
           (diminish 'which-key-mode " 🖮")))
  #+end_src
* COMMENT Paradox Package Manager                                      :Paradox:
  :PROPERTIES:
  :TANGlE:   no
  :END:
  #+begin_src emacs-lisp
...
* smart-mode-line
  #+begin_src emacs-lisp
    (use-package smart-mode-line
      ...
      ;; Short string replacement for the UFC45 folders
      (add-to-list 'sml/replacer-regexp-list '("^:CVS:ufc45docs/" ":UFC45DOCS:") t)
      (add-to-list 'sml/replacer-regexp-list '("^:CVS:ufc45sims/" ":UFC45SIMS:") t)
      (add-to-list 'sml/replacer-regexp-list '("^:UFC45DOCS:Reports/TR0\\([1-9]\\)" ":TR0\\1:") t)
      (add-to-list 'sml/replacer-regexp-list '("^:UFC45SIMS:MATLAB/wp3sims/" ":WP3SIMS:") t)
      )
  #+end_src
